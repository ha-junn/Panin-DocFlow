...
function getPageHref(page: number) {
  return page > 1 ? `/outgoing?page=${page}` : "/outgoing";
}
...
export default async function OutgoingPage({ searchParams }: OutgoingPageProps) {
  ...
  const { data, error, count } = await supabase
    ...
    .select(
      "id, agenda_number, sent_at, sender_staff, sender_department, letter_number, destination_name, attention_to, subject, confidential",
      { count: "exact" },
    )
  ...
  const nextHref = safeCurrentPage < totalPages ? getPageHref(safeCurrentPage + 1) : null;
                  ...
                  {rows.length > 0 ? (
                    ...
                    <tr>
                      <td colSpan={10} className="px-5 py-14 text-center">
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          Tambahkan data surat keluar pertama dari tombol di
                          kanan atas.
                        </p>
                      </td>
                    </tr>
